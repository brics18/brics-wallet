...
class KarlsenSendDialogMobile extends KarlsenDialog{
	...
	renderBody({estimating, estimateFee}){
		return html`
			...
			<flow-input class="amount full-width" suffix-btn outer-border
				label="${T('Amount in BRICS')}" @keyup=${this.onAmountChange}>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${KLS(dataFee)} BRICS</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${KLS(fee)} BRICS</td></tr>`:''}
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${KLS(totalAmount)} BRICS</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in BRICS", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] BRICS) is very large.');
