...
class KarlsenOpenDialog extends KarlsenDialog{
	...
	static get styles(){
		return [KarlsenDialog.styles, css`
			...
			.big-logo{max-width:150px;margin:10px auto 20px;display:block;border-radius:50%}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to BRICS Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="${baseUrl+'/resources/images/brics.png'}" />
			</div>`}
	...
	_renderOpenButtons(){
		return html`
			...
			<flow-btn primary @click="${this.openWallet}" i18n>OPEN BRICS WALLET</flow-btn>`;
	}
	renderCreateButtons(){
		return html`
