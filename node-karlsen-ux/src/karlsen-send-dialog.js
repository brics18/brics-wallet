...
class KarlsenSendDialog extends KarlsenDialog{
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'brics' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${KLS(totalAmount)} BRICS</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] BRICS) is too large.');
