...
border-radius: 50%;
	...
	render(){
		...
		let donationAddresses = [
			["BRICS Devfund donations:", "brics:qzljjhn97yjv74zl57pvwxnk0qnusy3mg4xtcnua0ylgarqhfpf76fzeyazjx"],
			["BRICS WebWallet donations:", "brics:qzljjhn97yjv74zl57pvwxnk0qnusy3mg4xtcnua0ylgarqhfpf76fzeyazjx"],
		]
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">BRICS WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the BRICS ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Brics Core BRICS:</span> ${window.PWA_MODULES['@karlsen/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Wallet Framework BRICS:</span> ${window.PWA_MODULES['@karlsen/wallet']}</div>
							<div class="badge"><span is="i18n-span">BRICS gRPC:</span> ${window.PWA_MODULES['@karlsen/grpc']}</div>
							<div class="badge"><span is="i18n-span">BRICS gRPC Relay:</span> ${window.PWA_MODULES['@karlsen/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Brics UX BRICS:</span> ${window.PWA_MODULES['@karlsen/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX BRICS:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">BRICS FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] BRICS</b> available.', BRICS(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${KLS(inUseUTXOs.satoshis||0)} BRICS</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="${baseUrl+'/resources/images/brics.png'}"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.brics.info/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatKLS(total)+' BRICS':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatKLS(available)} BRICS</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatKLS(pending)} BRICS</span>
					</div>
				`}
            </div>
